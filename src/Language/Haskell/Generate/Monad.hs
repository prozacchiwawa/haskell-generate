...
newtype Generate a = Generate { unGenerate :: StateT Integer (Writer (S.Set (ModuleName SrcLoc))) a } deriving (Functor, Applicative, Monad)
...
instance GenExp Char where
  ...
  expr ch = return $ Expression $ (Lit noLoc) $ (Char noLoc) ch (show ch)

instance GenExp Integer where
  type GenExpType Integer = Integer
  expr i = return $ Expression $ (Lit noLoc) $ (Int noLoc) i (show i)

instance GenExp Rational where
  type GenExpType Rational = Rational
  expr f = return $ Expression $ (Lit noLoc) $ (Frac noLoc) f (show f)
...
#elif MIN_VERSION_haskell_src_exts(1,20,3)
exportFun :: FunRef t -> ExportSpec SrcLoc
...
#if MIN_VERSION_haskell_src_exts(1,20,3)
  Module noLoc (Just (ModuleHead noLoc (ModuleName noLoc name) Nothing (Just (ExportSpecList noLoc $ fromMaybe [] export)))) [] (map (\md -> ImportDecl noLoc md True False False Nothing Nothing Nothing) $ S.toList imps) decls
#elif MIN_VERSION_haskell_src_exts(1,16,0)
